...
const CFOAllUsersGoals = () => {
  ...
  const handleStatusUpdate = async (goalId, goalStatus) => {
    try {
      ...
      setAllGoals((prev) =>
        prev.map((g) => (g.GoalID === goalId ? { ...g, GoalStatus: goalStatus } : g))
      );
    } catch (err) {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {allGoals.length > 0 ? allGoals.map((goal) => (
                <tr key={goal.GoalID}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{goal.GoalTitle}</td>
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-4 text-center text-sm text-gray-500">No organization data found.</td>
                </tr>
              )}
            </tbody>
          </table>
