...
const TeamManagement = () => {
  ...
  // 1. Fetch team members reporting to logged-in HOD/Manager/CFO
  useEffect(() => {
    fetchTeamUsers();
  }, []);
  ...
  return (
    <div className="p-6">
      ...
      {/* Team Users Table with Horizontal Scroll */}
      {!selectedUser ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">User Name</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">Post / Role</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">Submitted Goals</th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">Action</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-12 text-center text-sm text-gray-500">Loading team members...</td>
                  </tr>
                ) : users.length > 0 ? (
                  users.map((user) => (
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-3">
              {/* New: entry point into the Final Evaluation page (built
                  alongside evaluationController.js) — uses react-router
                  Link/navigate rather than this file's existing hardcoded
                  fetch() calls, since those already point at the wrong
                  base path (http://localhost:5000/api/... instead of
                  .../api/v1/...) and are a separate pre-existing bug,
              ...
              <button
                onClick={() => { setSelectedUser(null); setGoals([]); }}
          ...
          {/* Goals Table with Horizontal Scroll */}
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50/75">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Goal No / Title</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Category</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Weightage</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Priority</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {loading ? (
                    <tr>
                      <td colSpan="6" className="px-6 py-12 text-center text-sm text-gray-500">Loading goals...</td>
                    </tr>
                  ) : goals.length > 0 ? (
                    goals.map((goal) => (
                      <tr key={goal.GoalID} className="hover:bg-gray-50/50 transition">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                          ...
                          <Link to={`/goals/review?goalId=${goal.GoalID}`} className="text-emerald-600 hover:text-emerald-900 font-semibold bg-emerald-50 px-3 py-1.5 rounded-md transition hover:bg-emerald-100">Review</Link>
                        </td>
                      </tr>
                    ))
                  ) : (
