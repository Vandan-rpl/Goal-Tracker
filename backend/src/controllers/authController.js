    ...
    try {
        ...
        if (toBool(user.IsLocked)) {
            return res.status(403).json({
                success: false,
                message: 'Your account has been locked due to multiple failed login attempts. Please contact admin.'
            });
        }
        ...
        if (!isMatch) {
            await pool.request()
                ...
                .query('UPDATE Users SET FailedLoginCount = FailedLoginCount + 1 WHERE UserID = @UserID');
        ...
        return res.status(200).json({
            success: true,
            message: !isPasswordChanged ? 'Login successful. Password reset required.' : 'Login successful.',
            data: {
                token,
                isPasswordChanged,
                defaultPasswordFlag,
...
const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const userResult = await pool.request()
            ...
            .query('SELECT TOP 1 UserID, Username, Email FROM Users WHERE Email = @Email AND IsActive = 1');
        ...
        return res.status(500).json({ success: false, message: 'Internal server error while sending reset email.' });
    }
};
