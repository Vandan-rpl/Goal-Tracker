    ...
    try {
        ...
        if (role === 'BUSINESSHEAD') {
            query = `
                ...
                FROM dbo.Users u
                WHERE u.BusinessHeadID = @ManagerID AND u.UserID <> @ManagerID
            `;
        } else {
            query = `
                SELECT u.UserID, u.FirstName, u.LastName, u.Designation, u.Role,
...
const updateStatus = async (req, res) => {
    try {
        ...
        const goalInfoResult = await pool.request()
        ...
        const { UserID: goalOwnerId, GoalTitle: goalTitle } = goalInfoResult.recordset[0];
        ...
        if (goalStatus && (goalStatus.includes('Approved') || goalStatus.includes('Rejected'))) {
            try {
                ...
                console.error(`[NOTIF FAILED] teamController.updateStatus notify step. goalId=${goalId} userId=${goalOwnerId}:`, notifyErr);
            }
        }
