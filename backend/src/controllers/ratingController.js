...
const getGoalWithOwner = async (pool, goalId) => {
  const result = await pool.request().input("GoalID", sql.BigInt, goalId)
    .query(`
            SELECT
                g.GoalID,
                g.GoalTitle,
...
const submitQuarterlyUpdate = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[NOTIF FAILED] submitQuarterlyUpdate notify step. goalId=${goalId} userId=${goal.HODID}:`,
        notifyErr,
      );
    }
...
const submitHODRating = async (req, res) => {
  ...
  try {
    ...
    const isAuthorizedRater =
      goal.HODID === hodId || goal.ReportingManagerID === hodId;
    if (!isAuthorizedRater) {
      return res.status(403).json({
        success: false,
        message:
          "Only the HOD/reporting manager of this goal owner can submit an HOD rating.",
      });
    }
    ...
    await pool
      ...
      .input("Feedback", sql.VarChar, feedback || "").query(`
    ...
    try {
      ...
      console.error(
        `[NOTIF FAILED] submitHODRating notify step. goalId=${goalId} userId=${goal.OwnerID}:`,
        notifyErr,
      );
    }
...
const getQuarterlyUpdates = async (req, res) => {
  ...
  try {
    ...
    // Match the goal-view hierarchy: the owner and their direct approvers can
    // view progress. CFOs and admins can view it as part of their oversight.
    const requesterRole = String(req.user?.role || req.user?.Role || "").toLowerCase();
    const requesterId = Number(userId);
    const isAuthorized =
      Number(goal.OwnerID) === requesterId ||
      Number(goal.HODID) === requesterId ||
      Number(goal.ReportingManagerID) === requesterId ||
      Number(goal.BusinessHeadID) === requesterId ||
      ["admin", "cfo"].includes(requesterRole);

    if (!isAuthorized) {
      return res.status(403).json({
        success: false,
        message:
          "You are not authorized to view quarterly updates for this goal.",
      });
    }
